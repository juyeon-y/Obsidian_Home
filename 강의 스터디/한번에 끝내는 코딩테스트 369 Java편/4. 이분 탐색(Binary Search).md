...
- 원래 문제 : `C개의 공유기를 설치했을 때`, 최대 `인접 거리`는 얼마?
- 뒤집은 문제 : 어떤 `거리`만큼은 거리를 둘 때, `공유기 C개 설치 가능?` -> Yes/No
- 구현 : 
``` java
import java.io.*;
import java.util.*;
...
public class Main {
    ...
    // 2 <= 집의 개수 N <= 20만, 2 <= 공유기 개수 C <= N, 0 <= 집의 좌표 <= 십억
    // 최대최소치 : 공유기 사이 최대 거리 10억 -> int
    // 시간복잡도 : 집 배열 정렬 O(NlogN), 결정문제 O(N), 공유기 사이 최대 거리 이분탐색 O(logX) 소요 -> 결론적으로 O(NlogN + NlogX). 최대는 20만 * (log 20만 + log 10억) = 약 920만
    ...
    public static void main(String[] args) {
        // 1) 결정문제 : 가장 인접한 두 공유기 사이 거리가 X라고 할 때, C개의 공유기를 모두 설치 가능한가?
        // 2) 정렬인지 확인 : YYY..|NNN... -> 정렬 O
        // 가장 마지막 Yes를 찾아야 함
        input();
