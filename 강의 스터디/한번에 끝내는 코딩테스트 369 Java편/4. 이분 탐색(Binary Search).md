...
- 참고) 3번 질문은 풀이가 1, 2와 다름 주의. 1, 2는 구하는 인덱스가 하나로 특정되지만, 3은 X와 가장 가까운 앞 뒤 2개의 원소 인덱스 구해 둘 중 누가 더 가까운지 확인하는 식으로 해결. (ex. X=3, {1, 2, 6, 7} 일 경우, 3이상인 것중 가장 왼쪽에 있는 6과 [6의 인덱스 - 1] 원소인 2 중 누가 3과 더 가까운지 확인)
	- -> 정렬 특성 중, "각 원소마다, 그 원소와 가장 가까운/비슷한 원소는 해당 원소의 양 옆에 있는 원소 중에 존재한다" 사용
