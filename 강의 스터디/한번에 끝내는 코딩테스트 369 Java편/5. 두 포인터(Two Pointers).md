...
- 두 포인터 문제 종류 :
	1. 1차원 배열 위에 2개의 포인터를 만드는 경우
		1) 2개의 포인터가 모두 `왼쪽`에서 시작해서 `같은 방향`으로 이동
		2) 2개의 포인터가 `양 끝`에서 `서로를 향해` 이동
...
- 접근법 : 
	1. 최소 + 최대 < 0
		-> 최소(L) 입장에서는 최선책을 만난 상태(최대보다 더 작은 수를 더한다한들 0으로부터 멀어지기만 함). 짝을 찾았으니 최소는 삭제하고 더 고려 X
	2. 최소 + 최대 > 0
		-> 최대(R) 입장에서는 최선책을 만난 상태(최소보다 더 큰 수를 더한다한들 0으로부터 멀어지기만 함). 짝을 찾았으니 최대는 삭제하고 더 고려 X
	3. 최소 + 최대 == 0
		-> 최소(L), 최대(R) 모두 최선책을 만난 상태이므로 둘다 삭제하고 고려 X
	4. 남은 원소가 1개(L == R)가 되거나 없을 경우(R<L), 서로 다른 두 용액을 고를 수 없는 상태이므로 종료

- 시간복잡도 : 
	- 정렬 X 풀 경우 : 
		1. 매 순간 새로 L, R 을 찾아야 함 -> $O(N)$
		2. 남은 원소가 1개가 될 때까지 반복 -> N번 반복
		3. 총 $O(N^2)$의 시간 걸림

	- 정렬 O 풀 경우 :
		1. 배열 정렬 한 번 -> $O(NlogN)$
		2. L, R 로 계산한 후에 남은 원소가 1개가 될 때까지 이동 => $O(N)$
...
public class Main {
    ...
    // 2 <= 전체 용액 수 N <= 10만, -10억 <= 용액 특성값 <= 10억
    // 최대최소값 : -20억 <= 두 용액 합 <= 20억
    // 시간복잡도 : 정렬 O(NlogN) + 투 포인터 O(N). -> 결국 O(NlogN). 최대 10만 * log 10만 = 약 160만
    ...
    static void solution() {
        // 1. 정렬
        Arrays.sort(arr);
        // 2. 투 포인터 L, R : 합 > 0 경우 R--, 합 < 0 경우 L++, L >= R일 경우 종료
        int L = 0, R = N - 1, sumClosestToZero = Integer.MAX_VALUE, liq1 = 0, liq2 = 0;
        while(L < R) {
            int cur = arr[L] + arr[R];
            if(Math.abs(cur) < Math.abs(sumClosestToZero)) {
                sumClosestToZero = cur;
                liq1 = arr[L];
                liq2 = arr[R];
            }
            if(cur >= 0) R--;
        ...
        sb.append(liq1).append(' ').append(liq2);
